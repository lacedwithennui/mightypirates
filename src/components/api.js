...
export async function postImages(imageElement, featured) {
    ...
    for (let i = 0; i < imageElement.files.length; i++) {
        ...
        reader.onload = async () => {
            ...
            await fetch(`${process.env.REACT_APP_DB_BASE_URL}/images`,
                {
                    method: "POST",
                    headers: {
...
export async function getImages(imageIDsArray) {
    ...
    for(let i = 0; i < imageIDsArray.length; i++) {
        let response = await fetch(`${process.env.REACT_APP_DB_BASE_URL}/images/` + imageIDsArray[i]["$oid"]);
...
export async function submitPost(event) {
    ...
    fetch(`${process.env.REACT_APP_DB_BASE_URL}/posts`, {
        method: "POST",
        headers: {
            Authorization: "Bearer " + new Cookies().get("token")
        },
...
export async function auth(event, username, password) {
    ...
    fetch(`${process.env.REACT_APP_DB_BASE_URL}/auths`, {
        method: "GET",
        headers: {
...
export function redirectIfBadAuth() {
    fetch(`${process.env.REACT_APP_DB_BASE_URL}/auths/check`, {
        headers: {
            Authorization: "Bearer " + new Cookies().get("token")
        }
    })
...
export async function AllMeetings() {
    let response = await fetch(`${process.env.REACT_APP_DB_BASE_URL}/posts`);
    let json = await response.json();
    let allDays = [];
